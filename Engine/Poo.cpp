...
void Poo::Init(int x_init, int y_init, int vx_init, int vy_init)
{
...
void Poo::Update() // neither const nor by ref because this IS a mutator function
{
...
bool Poo::TestCollision( const Dude& dude ) const // call by const ref because function is read only
{
	assert(initialized == true);
...
void Poo::Draw( Graphics& gfx) const // const member function (non-mutator) because it does not change Poo or its members
{
	assert(initialized == true);
